...
│   ├── learning-resources.md← 学习资料来源、版权边界、资料策略
│   ├── question-bank-design.md← 阅读/听力题库结构与判分设计
...
│           ├── date.js      ← 日期工具函数
│           ├── score.js     ← 雅思分数换算工具
│           ├── vocab-data.js← 合并后的应用词汇数据（精选词条优先）
│           ├── ielts-4000-data.js← 导入的完整 IELTS 词库
│           └── question-bank-data.js← 阅读/听力题库示例数据
├── assets/
│   ├── icons/               ← PWA 图标（各尺寸）
│   └── audio/               ← 听力音频（本地示例）
├── scripts/
...
**截至 2026-07-02**：项目基座、同步闭环与完整词库已可运行

| 阶段 | 状态 | 说明 |
|------|------|------|
| 阶段 0：项目基座 | ✅ 已完成 | 文件结构、docs、README、PWA、本地服务、日志脚本 |
| 阶段 1：MVP（计划+词汇） | ✅ 已完成扩展版 | 备考计划生成、今日任务、SM-2 词汇复习、薄弱词反馈、掌握/未掌握板块、完整 IELTS 英汉词库 |
| 阶段 2：四科训练记录 | ✅ 已完成基础版 | 训练记录、错题本、模考记录 |
| 阶段 3：刷题系统 | 🔧 设计中 | 题库数据结构、原创阅读/听力样例已建立 |
| 阶段 4：阅读练习 | 🔧 设计中 | 原创 Passage 样例与题型字段已建立 |
| 阶段 5：听力练习 | 🔧 设计中 | 原创 transcript 样例与题型字段已建立 |
...
内置词汇表位于 `frontend/js/utils/vocab-data.js`，完整导入词库位于 `frontend/js/utils/ielts-4000-data.js`。当前已接入完整 IELTS 词库，并用 ECDICT 补充中文释义和音标；精选中文词条优先覆盖导入词条。后续扩展时优先使用可追踪来源：
